...
# -*- coding: utf-8 -*-
"""
基于Kronos模型的VGT ETF预测系统
使用Kronos-small模型对VGT ETF进行1天、3天、5天的涨跌预测
"""
...
class KronosVGTPredictor:
    """
    基于Kronos模型的VGT ETF预测器
    """
    ...
    def create_visualizations(self, analysis_results, save_path="Result-vgt"):
        """
        创建可视化图表
    ...
    def _plot_price_and_scores(self, results_df, ohlc_data, save_path):
        ...
        # 价格走势
        axes[0].plot(ohlc_data['date'], ohlc_data['close'], label='VGT Close Price', linewidth=1)
        axes[0].set_title('VGT ETF Price Trend', fontsize=14, fontweight='bold')
...
def main():
    ...
    """
    print("="*60)
    print("Kronos模型VGT ETF预测系统")
    print("="*60)
    
    # 初始化预测器
    predictor = KronosVGTPredictor()
    
    # 运行预测分析
    data_path = "Data/etf_VGT_2025-09.csv"
    ...
    # 保存预测结果
    results_df = analysis_results['results']
    results_df.to_csv('Result-vgt/kronos_prediction_results.csv', index=False)
    print("预测结果已保存到 Result-vgt/kronos_prediction_results.csv")

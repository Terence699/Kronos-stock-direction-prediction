...
class ScoreEnsemble:
    ...
    def load_all_scores(self):
        ...
        os.makedirs(os.path.dirname(str(AAPL_SCORES)), exist_ok=True)
        os.makedirs(os.path.dirname(str(VGT_SCORES)), exist_ok=True)
        os.makedirs(os.path.dirname(str(SENTIMENT_SCORES)), exist_ok=True)
        
        # Load AAPL scores
        aapl_file = str(AAPL_SCORES)
        if os.path.exists(aapl_file):
            ...
            try:
                ...
                if self.aapl_scores is not None:
                    ...
                    # persist
                    self.aapl_scores.to_csv(str(AAPL_SCORES), index=False)
                else:
        ...
        if os.path.exists(vgt_file):
            ...
            try:
                ...
                if self.vgt_scores is not None:
                    ...
                    # persist
                    self.vgt_scores.to_csv(str(VGT_SCORES), index=False)
                else:
        ...
        sentiment_file = str(SENTIMENT_SCORES)
        if os.path.exists(sentiment_file):
            ...
            try:
                ...
                if self.sentiment_scores is not None:
                    ...
                    # persist
                    self.sentiment_scores.to_csv(str(SENTIMENT_SCORES), index=False)
                else:
    ...
    def save_results(self):
        ...
        # Save summary report
        summary_file = str(ENSEMBLE_SUMMARY)
        ...
        # Save grid search numeric results to plain text for custom viz
        try:
            grid_txt = os.path.join(os.path.dirname(summary_file), 'grid_search_results.txt')
            with open(grid_txt, 'w') as f:
                f.write("GRID SEARCH RESULTS (per horizon)\n")
                f.write("="*60 + "\n\n")
                for horizon in ['1d', '3d', '5d']:
                    if horizon in self.grid_search_results:
                        best_cv = self.grid_search_results[horizon]['best_cv_score']
                        best_w = self.grid_search_results[horizon]['best_weights']
                        f.write(f"Horizon: {horizon}\n")
                        f.write(f"Best CV Score: {best_cv:.4f}\n")
                        f.write(f"Best Weights: AAPL={best_w['aapl_weight']:.3f}, "
                                f"VGT={best_w['vgt_weight']:.3f}, "
                                f"Sentiment={best_w['sentiment_weight']:.3f}\n")
                        f.write("-"*60 + "\n")
                        f.write(f"{'AAPL':<8}{'VGT':<8}{'SENT':<8}{'CV':<8}\n")
                        for r in self.grid_search_results[horizon]['all_results']:
                            w = r['weights']
                            f.write(f"{w['aapl_weight']:<8.3f}{w['vgt_weight']:<8.3f}{w['sentiment_weight']:<8.3f}{r['cv_score']:<8.4f}\n")
                        f.write("\n")
            print(f"Grid search results saved to: {grid_txt}")
        except Exception as e:
            print(f"Warning: failed to write grid search text results: {e}")

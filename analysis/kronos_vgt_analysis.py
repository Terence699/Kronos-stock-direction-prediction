#!/usr/bin/env python3
"""
VGT ETF Analysis using Kronos Mini - Independent Scoring Module
==============================================================

This module performs scoring analysis of VGT ETF data using:
...
from analysis.kronos_model import KronosPredictor as RealKronosPredictor
from config import VGT_FILE
...
class TechnicalIndicators:
    """Calculate technical indicators for ETF analysis"""
    ...
    def calculate_all_indicators(self):
        ...
        print("VGT technical indicators calculated successfully!")
...
class KronosPredictor:
    ...
    def load_model(self):
        ...
        try:
            ...
            print("Loading Kronos mini model for VGT...")
            self.model_loaded = True
            print("VGT model loaded successfully!")
        except Exception as e:
            print(f"Error loading VGT model: {e}")
            self.model_loaded = False
...
class RollingAnalysis:
    ...
    def __init__(self, data, lookback_window=60, use_real_kronos=False):
        ...
        if use_real_kronos:
            print("Using REAL Kronos model implementation for VGT...")
            self.predictor = RealKronosPredictor(device="cpu", offline_mode=True)  # Use offline mode
        else:
            print("Using enhanced Kronos predictor with unified features for VGT...")
    ...
    def run_rolling_analysis(self):
        ...
        print(f"Starting enhanced rolling analysis for VGT with {self.lookback_window}-day window...")
        print("🎯 Key Features:")
        ...
        print(f"VGT rolling analysis completed! Processed {len(self.results)} windows.")
...
class VGTScoreGenerator:
    """Independent VGT scoring module for ensemble learning"""
    
    def __init__(self, data_path=None):
        self.data_path = str(data_path or VGT_FILE)
        self.results_df = None
        self.scores = None
        
    def load_data(self):
        """Load and preprocess VGT data"""
        print("Loading VGT ETF data...")
        ...
        print(f"VGT data loaded: {len(data)} records from {data['date'].min()} to {data['date'].max()}")
        return data
    ...
    def generate_scores(self):
        ...
        try:
            ...
            # Calculate scores
            print("Calculating VGT model scores...")
            ...
            print("VGT scoring completed successfully!")
            return standardized_scores
            
        except Exception as e:
            print(f"Error in VGT scoring: {e}")
            import traceback
            traceback.print_exc()
            return None
    ...
    def _standardize_scores(self):
        ...
        for horizon in ['1d', '3d', '5d']:
            ...
            if pred_col in self.results_df.columns:
                ...
                standardized_df[f'vgt_pred_{horizon}'] = (self.results_df[pred_col] * 2 - 1)
                
                # Add actual direction for evaluation
                if actual_col in self.results_df.columns:
                    standardized_df[f'vgt_actual_{horizon}'] = self.results_df[actual_col]
                
                # Add price change for evaluation
                if price_change_col in self.results_df.columns:
                    standardized_df[f'vgt_return_{horizon}'] = self.results_df[price_change_col]
...
def main():
    ...
    print("VGT ETF Analysis using Kronos Mini - Independent Module")
    print("="*60)
    
    # Initialize VGT score generator
    vgt_generator = VGTScoreGenerator()
    
    # Generate scores
    scores_df = vgt_generator.generate_scores()
    
    if scores_df is not None:
        # Save results
        results_file = os.path.join(get_project_root(), 'vgt_scores.csv')
        scores_df.to_csv(results_file, index=False)
        print(f"VGT scores saved to '{results_file}'")
        
        # Print performance metrics
        metrics = vgt_generator.get_performance_metrics()
        if metrics:
